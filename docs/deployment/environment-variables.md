...
This document lists every environment variable required to run the StellAdullam platform. All variables come directly from the `.env.example` file. Any variable not listed here does not exist in the codebase and should not be referenced.
...
| ------------------- | ------------------------------------------------------- | --------------------- | --------------------------------------------------------- |
| `DATABASE_URL`      | `postgresql://user:password@localhost:5432/stelladullam_defi` | Yes                   | Full PostgreSQL connection string                         |
...
| Variable         | Example Value             | Required | Description                                                                                                                    |
| ---------------- | ------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `KYC_UPLOAD_DIR` | `/var/StellAdullam/kyc-uploads` | Yes      | Absolute path on the server where KYC document files are stored. The API process must have read/write access to this directory |
